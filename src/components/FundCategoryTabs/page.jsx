...
export default function FundCategoryTabs() {
  ...
  return (
    <div className=" ">
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 px-4 sm:px-6 md:px-8 lg:px-10">
  {tabs.map((tab, index) => (
    <button
      key={index}
      className={`flex items-center gap-2 p-4 rounded-t text-[16px] sm:text-[18px] md:text-[20px] font-semibold transition-all duration-300
        ${
          activeTab === tab
            ? "bg-[var(--rv-primary)] text-[var(--rv-white)] border-b-2"
            : "hover:bg-[var(--rv-primary)] hover:text-[var(--rv-white)] border-b-2"
        }`}
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="bg-[var(--rv-ternary)] rounded-full p-2">
                    <Image
                      src={`/icons/performance/${activeTab
                        .toLowerCase()
                        .replace(/\s+/g, "")}/${idx}.svg`}
                      alt={activeTab}
                      width={32}
                  ...
                  <div className="text-sm font-medium text-[var(--rv-white)]">
                    {item}
                  </div>
                </div>
