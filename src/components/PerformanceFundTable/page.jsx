...
import { FundListSkeleton } from "../skeletons/performanceSkeleton";
...
export default function MutualFundTable({ performanceData, schemeName }) {
  ...
  return (
    <div className="main_section ">
      <div className="max-w-screen-xl mx-auto bg-[#ffffff69] ">
        {/* Filters */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-3 p-4">
          <input
            type="text"
        ...
        {/* Fund List */}
        <div className="bg-[var(--rv-primary)] text-[var(--rv-white)] shadow rounded  overflow-hidden m-4">
              ...
              <>
                ...
                /> */}
                    <div>
                      ...
                      <p className="text-sm text-[#FFFFFFED]">
                        {fund.schemeCategory}
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    className="grid grid-cols-2 md:grid-cols-3 gap-2 items-center text-center w-full md:w-1/2"
                    onClick={() => handleSelectFunds(fund)}
                  >
                    <div className="md:text-right">
                      <p className="text-sm text-[#FFFFFFED]">Fund Size</p>
                    ...
                    >
                      {(fund?.fiveyear_navEndDate &&
                        fund.fiveyear_navEndDate !== "0.00" && (
                          <div>
                            <p className="text-sm text-[#FFFFFFED]">NAV</p>
                            <p className="font-medium text-gray-200">
                              ₹{fund?.fiveyear_navEndDate}
                            </p>
                          </div>
                        ...
                        (fund?.sixmonth_navEndDate &&
                          fund.sixmonth_navEndDate !== "0.00" && (
                            <div>
                              <h3 className="text-sm text-[#FFFFFFED]">NAV</h3>
                              <p className="text-lg font-medium text-gray-200">
                                ₹ {fund?.sixmonth_navEndDate}
                              </p>
                            </div>
                          )) /* Add six_month check here if available */ ||
                        (fund?.three_month && fund.three_month !== "0.00" && (
                          <div>
                            <h3 className="text-sm text-[#FFFFFFED]">
                              NAV: 6 Month Data
                            </h3>
                            <p className="text-lg font-medium text-gray-200">
                              ₹ {fund?.three_month}
                    ...
                    >
                      {(() => {
                        ...
                        const numFive = parseFloat(five_year);
                        const numThree = parseFloat(three_year);
                        const numOne = parseFloat(one_year);

                        if (five_year !== "0.00" && numFive >= 0) {
                          value = five_year;
                          label = "5Y";
                        } else if (three_year !== "0.00" && numThree >= 0) {
                          value = three_year;
                          label = "3Y";
                  ...
                  <div className="md:text-right">
                    <Link href="/contact-us" className="">
                      <span className="bg-[var(--rv-secondary)]  text-[var(--rv-white)]  font-bold mx-4 px-4 py-2 rounded-lg whitespace-nowrap transition-all">
                        Invest Now
                      </span>
                    </Link>
                  </div>
